...
enum class MyHostType(val position: Int, val title: String, val status: List<Int>) {
    ...
    PROCESS_SHOP(2, getString(R.string.process), listOf(OrderStatusType.GATHER_SUCCESS.status,OrderStatusType.ORDER_SUCCESS.status,OrderStatusType.SHOP_SHIPMENT.status)),
